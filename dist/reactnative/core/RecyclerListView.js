...
var RecyclerListView = /** @class */ (function (_super) {
    ...
    RecyclerListView.prototype.scrollToIndex = function (index, animate) {
        var layoutManager = this._virtualRenderer.getLayoutManager();
        if (layoutManager) {
            var layouts = layoutManager.getLayouts();

            if (layouts.length <= index) {
                throw new Error("Invalid index to scrollToIndex");
            }

            const itemLayout = layouts[index];

            const itemHeight = itemLayout.height;
            let offsetY = itemLayout.y;
            let offsetX = itemLayout.x;

            const currentScrollOffset = this.getCurrentScrollOffset();

            const layoutHeight = this._layout.height;

            const listEndY = currentScrollOffset + layoutHeight;

            const itemEndY = offsetY + itemHeight;

            if(offsetY >= currentScrollOffset && itemEndY <= listEndY) {
                return;
            }

            if(itemEndY > listEndY) {
                offsetY = currentScrollOffset + itemEndY - listEndY;
            }

            this.scrollToOffset(offsetX, offsetY, animate);
        }
        else {
